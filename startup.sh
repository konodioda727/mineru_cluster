...
print_effective_config() {
  ...
  if [[ "$(uname)" == "Darwin" ]]; then
    log "  OBJC_DISABLE_INITIALIZE_FORK_SAFETY=${OBJC_DISABLE_INITIALIZE_FORK_SAFETY:-YES (auto)}"
    log "  OMP_NUM_THREADS=${OMP_NUM_THREADS:-1 (auto)}"
    log "  PYTORCH_ENABLE_MPS_FALLBACK=${PYTORCH_ENABLE_MPS_FALLBACK:-1 (auto)}"
  fi
}
